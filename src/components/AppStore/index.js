...
class AppStore extends Component {
  ...
  setTabId = tabIdActve => {
    this.setState({activeTabId: tabIdActve})
  }
  ...
  getSearchResult = () => {
    ...
    const searchAppList = appsList.filter(eachapp =>
  ...
  getfilteredSearchList = searchResult => {
    const {activeTabId} = this.state
    const filteredResult = searchResult.filter(
      eachResult => eachResult.category === activeTabId,
    )
  ...
  render() {
    ...
    const filteredSearchList = this.getfilteredSearchList(searchResult)
